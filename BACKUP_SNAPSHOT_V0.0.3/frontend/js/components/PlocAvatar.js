...
export const createPlocAvatar = (containerId) => {
    ...
    const updateMode = (newMode, skipVoice = false) => {
        mode = newMode;
        clearInterval(zInterval);
        effectsLayer.innerHTML = '';
        ploc.style.animation = '';
        
        // Reset de posição e transição
        eyesContainer.style.transform = 'translate(0, 0)';
        eyesContainer.style.transition = 'all 0.3s ease';
        ...
        if (mode === 'sleeping') {
            ...
            startZzz();
            if (!skipVoice) speak("vou tirar um cochilo, precisando me acorde");
        } 
        else if (mode === 'working') {
            eyesElements.forEach(({ eye }) => {
                eye.style.height = '12px';
            });
            ploc.style.animation = 'plocWorkMove 1s ease-in-out infinite'; // Mais rápido para parecer focado
            if (!skipVoice) speak("Vou começar a trabalhar aqui, se quiser falar comigo me cucuta");
        } 
        else if (mode === 'active') {
            eyesContainer.style.transition = 'none'; // Instantâneo para seguir o mouse
            eyesElements.forEach(({ eye, pupil }) => {
                eye.style.height = '20px';
                eye.style.width = '8px';
                eye.style.borderRadius = '10px';
                eye.style.background = '#1e293b';
                eye.style.border = 'none';
                pupil.style.opacity = '0';
            });
            if (!skipVoice) speak("Bom dia, me chamou?");
        }
    };

    // Lógica de Seguir o Mouse (Move as cápsulas inteiras)
    document.addEventListener('mousemove', (e) => {
        ...
        const dist = 6; // Intensidade do movimento
        const x = Math.cos(angle) * dist;
        const y = Math.sin(angle) * (dist / 2); // Menos movimento vertical para manter a elegância
