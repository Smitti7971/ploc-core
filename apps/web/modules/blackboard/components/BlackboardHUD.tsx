...
export function BlackboardHUD({
  ...
  useEffect(() => {
    // Procura o container que está na AuthCapsule
    setPortalTarget(document.getElementById('blackboard-portal-target'));
  }, []);
  ...
  // Se o portal não for encontrado (ex: SSR), renderiza normal no topo esquerdo
  if (!portalTarget) {
    return (
      <div className="fixed top-4 left-4 md:top-6 md:left-6 flex items-center z-hud pointer-events-none max-w-[calc(100vw-32px)]">
  ...
  return createPortal(hudContent, portalTarget);
}
